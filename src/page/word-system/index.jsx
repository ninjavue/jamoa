...
const IMAGE_PLACEHOLDER_SRC =
  "data:image/svg+xml," +
  encodeURIComponent(
    '<svg xmlns="http://www.w3.org/2000/svg" width="1" height="1" viewBox="0 0 1 1"><rect width="1" height="1" fill="#f0f0f0"/></svg>',
  );
...
const FILE_ID_SLASH_RE = /^[0-9a-f]+\/[0-9a-f]+$/i;

// Mundarijaga va ko‘rinishga kiritilmaydigan sarlavhalar (Ekspluatatsiya oqibatlari, Tavsiyalar va shu kabi)
const TOC_HIDDEN_LABELS = ["Ekspluatatsiya oqibatlari", "Tavsiyalar"];
...
const section2ObjectLinks = ["https://axborot-tizim.uz"];
...
const takeRiskRowsByHeight = (
  ...
  const safeMaxHeight = Math.max(
    180,
    Math.floor(maxHeightPx - layoutTolerance),
  );
...
const computeRiskLevelRowspanMeta = (rows) => {
  ...
  while (idx < safeRows.length) {
    ...
    let end = idx;
    while (end < safeRows.length && safeRows[end]?.type !== "resource")
      end += 1;
    ...
    while (i < end) {
      ...
      for (let k = i + 1; k < j; k++)
        meta[k] = { showLevel: false, rowSpan: 1 };
...
const SystemWord = () => {
  ...
  const newContentFlowRefs = useRef({});
  const newContentHydratedRef = useRef(false);
  ...
  const [workersTableRows, setWorkersTableRows] = useState([
    ["Aliyev A.", "", "Yetakchi mutaxassis"],
      ...
      imgElement.addEventListener(
        ...
        () => {
          ...
          setTimeout(() => {
            ...
            wrapper.className = "system-pasted-image-wrap";
            ...
            // new-content sahifada rasm .page-content ostida qolmasin, flow ichida bo‘lsin
            if (editableRoot?.classList?.contains("new-content")) {
              const flow = editableRoot.querySelector(".system-two-col-flow");
              if (flow && wrapper.parentElement === editableRoot) {
        ...
        imageFiles.forEach((file) => {
          ...
          imgElement.onload = () => {
            ...
            setTimeout(() => {
              ...
              // new-content sahifada rasm .page-content ostida qolmasin, flow ichida bo‘lsin
              if (targetEditable?.classList?.contains("new-content")) {
                const flow = targetEditable.querySelector(
                  ".system-two-col-flow",
                );
                if (flow && wrapper.parentElement === targetEditable) {
                  flow.appendChild(wrapper);
                }
              }
              editables.forEach((el) => {
  ...
  const getExpertById = async () => {
    try {
      ...
      if (res.status === METHOD.OK) {
        ...
        if (
          ...
          try {
            ...
            if (dataFromField8.tables && dataFromField8.objectLinks) {
              ...
              if (
                Array.isArray(dataFromField8.workersTable) &&
                dataFromField8.workersTable.length > 0
              ) {
                setWorkersTableRows(dataFromField8.workersTable);
              }
              if (
                typeof dataFromField8.performedBy === "string" &&
                dataFromField8.performedBy.trim() !== ""
              ) {
        ...
        const sanitizedVulnData = vulnDataWithFileIds
  ...
  const injectImageFileIdsIntoBlocks = (blocks, filesArray) => {
    ...
    const list = Array.isArray(filesArray)
      ? filesArray.flat().filter(Boolean)
      : [];
    ...
    return blocks.map((block) => {
      ...
      try {
        ...
        imgs.forEach((img) => {
          ...
          if (!entry) return;
          const fid = entry?.[1] ?? entry?.["1"];
          const sz = entry?.[2] ?? entry?.["2"];
          if (fid) {
            img.setAttribute("data-file-id", String(fid));
            if (sz != null && Number.isFinite(Number(sz))) {
              img.setAttribute(
                "data-file-size",
                String(Math.floor(Number(sz))),
              );
            }
          }
        });
  ...
  const sanitizePersistedImageHtml = (html, options = {}) => {
    if (!html || typeof html !== "string") return html;
    const { keepBlobIfHasFileId = false } = options;
    try {
      const parser = new DOMParser();
      const doc = parser.parseFromString(html, "text/html");
  ...
  const sanitizeLiveImageElement = (img, options = {}) => {
  ...
  const generateVulnHtml = (vulnData) => {
    ...
    // Yangi zaiflik qo'shilganda batafsil izoh (tasnif, oqibat, tavsiyalar) DOM'da ko'rinsin
    newContentHydratedRef.current = false;
  ...
  const paginateContent = (items) => {
    ...
    measureContent.style.overflow = "visible";
    measureContent.style.margin = "0";
    measureContent.style.marginTop = `${Number.isFinite(contentOffsetTop) ? contentOffsetTop : 0
      }px`;
  ...
  const collectNewContentBlocksFromDom = () => {
    let allPages = document.querySelectorAll(
      ".word-pages .page-content.editable.new-content",
    );
    if (!allPages?.length) {
      allPages = document.querySelectorAll(
        ".word-container .page-content.editable.new-content",
      );
    }
      ...
      Array.from(normalizedChildren).forEach((child) => {
        ...
        if (clone.querySelectorAll) {
          clone
            ...
            .forEach((img) =>
              sanitizeLiveImageElement(img, { keepBlobIfHasFileId: true }),
            );
        }
        if (clone.tagName === "DIV") {
          const hasNestedDivs = clone.querySelector("div") !== null;
          const hasImportantClass =
            clone.classList.contains("text") ||
            clone.classList.contains("exp-title") ||
            clone.classList.contains("exp-d") ||
            clone.classList.contains("title");
          if (hasNestedDivs && !hasImportantClass) {
            pageBlocks.push(clone.innerHTML);
          } else {
    ...
    return allBlocks
      ...
      .map((item) =>
        sanitizePersistedImageHtml(item, { keepBlobIfHasFileId: true }),
      )
      .map((item) => normalizeNewContentBlockHtml(item))
      .flatMap((item) => explodeHtmlBlock(item))
  ...
  const hasImagesUploading = useCallback(() => {
    ...
    for (const img of imgs) {
      if (
        img.dataset?.uploadStarted === "true" &&
        !(img.dataset?.fileId || img.getAttribute("data-file-id"))
      )
        return true;
    }
    return false;
  ...
  const saveAllChanges = async () => {
    ...
    try {
      ...
      const rawBlocks = collectNewContentBlocksFromDom();
      // Serverga yuborishda blob/data URL ni placeholder ga almashtiramiz (base64 saqlamaymiz)
      const sanitizedForSave = rawBlocks.map((block) =>
        sanitizePersistedImageHtml(block, { keepBlobIfHasFileId: false }),
      );
      const pagedForServer = paginateContent(sanitizedForSave);
      // UI da blob qolishi uchun sahifalash blob li kontentdan
      const pagedForState = paginateContent(rawBlocks);

      // Table ma'lumotlarini o'qish; keepBlobInCells: true = UI (blob qoladi), false = server (placeholder)
      const extractTableData = (keepBlobInCells = false) => {
        const tables = document.querySelectorAll("table.editable-table");
        const data = {};
            ...
            const rowData = Array.from(cells).map((cell) => {
              ...
              if (images.length > 0) {
                return sanitizePersistedImageHtml(cell.innerHTML, {
                  keepBlobIfHasFileId: keepBlobInCells,
                });
              }
              return cellText;
            });
      ...
      const tablesForServer = extractTableData(false);
      const tablesForState = extractTableData(true);
      ...
      // Workers jadvalini DOM'dan o'qib saqlash (tahrir rejimida)
      let workersToSave = workersTableRows;
      const workersTbody = document.querySelector("table.workers-table tbody");
      if (workersTbody) {
        const rows = [];
        workersTbody.querySelectorAll("tr").forEach((tr) => {
          const cells = Array.from(tr.querySelectorAll("td")).map((td) =>
            (td.textContent || "").trim()
          );
          rows.push(cells);
        });
        if (rows.length > 0) workersToSave = rows;
      }

      // "Bajardi va chop etdi..." matnini DOM'dan o'qib saqlash
      let performedByToSave = performedByText;
      const performedByEl = document.querySelector(
        ".performed-by-text",
      );
      if (performedByEl) {
        const t = (performedByEl.textContent || "").trim();
      ...
      // UI da rasm doim blob bo‘lib qoladi: state ga blob li kontentni qo‘yamiz (serverga placeholder yuborildi)
      newVulnSnapshotRef.current = rawBlocks;
      setNewVuln(rawBlocks);
      setPages3(pagedForState);
  ...
  const handlePasteImage = async (file, imgElement) => {
    try {
      ...
      if (fileId && imgElement) {
        imgElement.setAttribute("data-file-id", fileId);
        imgElement.dataset.fileId = fileId;
        imgElement.dataset.uploaded = "true";
        if (safeUploadedSize) {
          const sizeStr = String(safeUploadedSize);
          imgElement.setAttribute("data-file-size", sizeStr);
          imgElement.dataset.fileSize = sizeStr;
        }
        imgElement.dataset.srcResolved = "true";
      }
  ...
  useEffect(() => {
    ...
    const run = async () => {
      ...
      const imgs = Array.from(
        container.querySelectorAll(".page-content img, .editable-table img"),
      );
      ...
      for (const img of imgs) {
        if (cancelled) return;
        if (!img) continue;
        // word/index.jsx kabi: allaqachon blob o‘rnatilgan bo‘lsa qayta yuklamaslik
        if (
        ...
        // Refresh/sahifa yuklanganda base64 ko‘rinmasin: data: (shu jumladan base64) ni darhol placeholder ga almashtiramiz
        if (currentSrcRaw.startsWith("data:") && currentSrcRaw !== IMAGE_PLACEHOLDER_SRC) {
          img.setAttribute("src", IMAGE_PLACEHOLDER_SRC);
        }
        let dfidRaw =
          img.getAttribute("data-file-id") || img.dataset.fileId || "";
        ...
        if (!dfidRaw) {
          // Rasmda file-id yo‘q bo‘lsa ham src bo‘sh qolmasin (broken icon o‘rniga placeholder)
          img.setAttribute("src", IMAGE_PLACEHOLDER_SRC);
          continue;
        }
        ...
        let size = Number.isFinite(Number(metaSize))
          ? Number(metaSize)
          : Number.isFinite(attrSize)
            ? attrSize
            : undefined;
        // Size bo‘lmasa ham (eski saqlangan hujjat) blob ni yuklab berish uchun fallback (5MB)
        const FALLBACK_IMAGE_SIZE = 5 * 1024 * 1024;
        if (!(Number.isFinite(size) && size > 0)) {
          size = FALLBACK_IMAGE_SIZE;
          imageLog("resolveStoredImages using fallback size", { dfid });
        }
        ...
        try {
          ...
          img.src = url;
          img.dataset.srcResolved = "true";
          // id bilan birga size ham img da saqlansin, keyingi yuklab olishda size kerak
          img.setAttribute("data-file-size", String(Math.floor(size)));
    ...
    // Sahifa yuklanganda DOM chizilguncha kutib, keyin rasmlarni blob ga aylantiramiz
    Promise.resolve()
      .then(() => new Promise((r) => requestAnimationFrame(r)))
      .then(run);
    return () => {
      cancelled = true;
      // Tahrirga o'tganda revoke qilmaymiz – blob URL tahrir rejimida ham ko'rinsin
      // Blob URL larni faqat komponent unmount bo'lganda revoke qilamiz (pastdagi useEffect)
    };
  }, [editing, uploadedFilesMeta, pages3, domRenderRevision]);
  ...
  const getActiveA4PageElement = useCallback(() => {
    ...
    if (selectedA4PageRef.current?.isConnected)
      return selectedA4PageRef.current;
    return null;
  }, []);
  ...
  // Tahrir rejimida new-content flow ni faqat bir marta state dan to‘ldiramiz; keyin re-render da DOM ustiga yozilmasin (rasm/matn yo‘qolmasin)
  useEffect(() => {
    if (!editing) {
      newContentHydratedRef.current = false;
      return;
    }
    if (newContentHydratedRef.current) return;
    const pages = detailPages || [];
    pages.forEach((pageItems, pageIndex) => {
      const flow = newContentFlowRefs.current[pageIndex];
      if (flow && Array.isArray(pageItems) && pageItems.length > 0) {
        flow.innerHTML = pageItems.map((item) => `<div>${item}</div>`).join("");
      }
    });
    newContentHydratedRef.current = true;
  }, [editing, detailPages]);
        ...
        >
            ...
            currentPages.map((pageItems, pageIndex) => (
              <div key={pageIndex} className="a4 system-c">
                ...
                <div className="page-content editable new-content">
                  {editing ? (
                    <div
                      className="system-two-col-flow"
                      ref={(el) => {
                        if (el) newContentFlowRefs.current[pageIndex] = el;
                      }}
                    />
                  ) : (
              ...
              <div>
                <div className="system-single">
                  ...
                  <p className="system-note">
                    Ekspertiza hisoboti {new Date().getFullYear()}-yil {new Date().getDate()}-{getExpMonth()} kunida olingan
          ...
          <div className="a4 system-c">
            ...
            <div className="page-content editable relative h-full">
              ...
              <div className="mt-10">
                <table
                  style={{ border: "none", marginLeft: "10px", marginTop: "30px" }}
                  className="w-full workers-table"
                >
                  <tbody>
                    {workersTableRows.map((row, rowIdx) => (
                      <tr
                        key={rowIdx}
                        className={rowIdx === 0 ? "h-[50px]" : "pt-5 h-[100px]"}
                      >
                        <td
                          className={`table-text ${rowIdx === 1 ? "min-w-[180px]" : ""}`}
                          contentEditable={editing}
                          suppressContentEditableWarning
                        >
                          {row[0]}
                        ...
                        <td className="table-text min-w-[160px]">
                          <div
                            className="w-[95%] min-w-[95%] border-b h-[20px] border-gray-700 min-h-[20px]"
                            contentEditable={editing}
                            suppressContentEditableWarning
                          >
                            {row[1]}
